 ...
 *      @brief  
 *
 *      Conversion from linear and angular velocities to wheel pulses. Subscriber to linear and angular velocities, 
 *      conversion of those values into a message containing Left/Right pulses to Publish into /client_messages (message for Arduino Leonardo ETH)
 *
...
int MSG_FREQ; // message frequency, defines the rate of the feedback loop, se the time in the velocity is relative to it
...
void chatterCallback(const r_platform::navi msg)
{
  ...
  //ROS_INFO("I heard: [%f] [%f] \n", msg.linear_vel, msg.angular_vel);
	...
	std_msgs::String msg_mot = generate_motor_message (msg.linear_vel, msg.angular_vel);
...
/**
 * 
 * @brief  Generation of message to the Arduino (in left/right pulses) given linear and angular velocities
